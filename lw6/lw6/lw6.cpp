...
DWORD WINAPI ThreadProc(CONST LPVOID lpParam)
{
	...
	LogBuffer* logBuffer = threadData->logBuffer;
	...
	for (int i = startBlurWidth; i < endBlurWidth; i++)
	{
		for (int j = 0; j < imageHeight; j++)
		{
			...
			std::string str = std::to_string(threadData->threadNumber) + "\t" + std::to_string(end_time - threadData->startTime) + "\n";
...
void Blur(const std::string input, const std::string output, const int threadCount, const int coreCount, const std::vector<ThreadPriority> threadPriorities)
{
	...
	LogFileWriter* logFileWriter = new LogFileWriter("LogWriter.txt");
